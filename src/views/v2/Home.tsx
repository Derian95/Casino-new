...
export const Home: FC<Props> = ({ animation }) => {
	...
	const scale = useTransform(scrollYProgress, [0, 0.5], [1, .8]);
	...
	return (
		<motion.div style={{ opacity }} ref={targetRef} className='w-full flex justify-center items-start  pt-44 px-5 relative mb-[8rem] h-screen  text-white before:pointer-events-none before:fixed before:inset-0 before:z-0 before:bg-[radial-gradient(circle_farthest-side_at_var(--x,_100px)_var(--y,_100px),#f5d6bf,_transparent_100%)] before:opacity-40 z-10"'>
			<motion.div   style={{ position, scale, x: "-50%" }} className='w-full lg:max-w-7xl flex-col flex justify-center items-center gap-8 z-20 fixed left-1/2 '>
